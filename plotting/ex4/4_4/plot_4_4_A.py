...
def load_mcperf_data():
    ...
    '''
    data_files = ["4_4_p95_QPS_1.csv", "4_4_p95_QPS_2.csv", "4_4_p95_QPS_3.csv"]
    lantecies_all = []
    qps_all = []
...
def format_plot(ax1, ax2):
    '''
        ...
        2 y axes
    '''
    # axes (ax1 = latency, ax2 = QPS)
    ax1.set_xlabel("Time [s]")
    ax1.set_ylabel("$\mathregular{95^{th}}$ percentile latency [ms]", rotation=0)
    ...
    ax1.set_yticks(np.arange(0, 1.6, 0.1), labels=None, minor=True)
    ax1.tick_params(axis='y', colors='royalblue')

    # set xticks
    ax1.set_xticks(np.arange(0, 850, 50), labels=None, minor=True)
    ax1.set_xticks(np.arange(0, 840, 100))
...
def format_schedule_plot(fig_ax):
    ...
    fig_ax.set_title("Plot A, run " + str(R) + ": Scheduling policy according to dynamic scheduler and variable load", loc='left', pad=40)

     # TODO: set x_axis limit to computed value
    fig_ax.set_xlim(-5, 850)
    ...
    # set x labels
    fig_ax.set_xticks(np.arange(0, 850, 50), labels=None, minor=True)
    fig_ax.set_xticks(np.arange(0, 840, 100))
...
def main():
    ...
    for i in range(len(job_order)):
        ...
        if job in cat1:
            ...
            # width of bars
            if len(start_times_norm) != len(end_times_norm):
                end_times_norm = end_times_norm[0:-1]
    ...
    #TODO: set x_axis limit to computed value
    #----------------------------------------------------------------------------------------------------------------
    # ---------------------------------plot latencies and qps (lower plot)------------------------------------------
    ax2 = ax[1].twinx()
    ...
    # plot latencies (ax1) and qps (ax2)
    ax[1].plot(x_vals, latencies[0:84, R-1], color='royalblue', marker='o', mec='white', mew=0.7, ms=4.5, linewidth=1.5, label='latency')
    ax2.plot(x_vals, qps[0:84, R-1], color='grey', marker='h', mec='white', mew=0.7, ms=4.5, linewidth=1.5, label='QPS', alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig('plot_4_4_A_' + str(R) + '.pdf')
