...
read_files=("new_log1.txt" "new_log2.txt" "new_log3.txt")
...
# pausing happens (swapped categories of vips and dedup from controller.py)
cat1=('radix' 'blackscholes' 'dedup')
...
for i in {1..3}
do
    data_file="new_tails${i}.txt"
    csv_filename="4_3_p95_QPS_${i}.csv"

    # file nr. 3 stored with tabs, not comma separated
    if [ $i -eq 3 ]
    then
        cat $data_file | grep 'read' | awk '{print $13 ", " $17}' > $csv_filename
    else
        cat $data_file | grep 'read' | awk -F"," '{print $13 ", " $17}' > $csv_filename
    fi
done
